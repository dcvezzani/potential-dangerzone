...
resource so far is JTestR's site itself: http://jtestr.codehaus.org/Getting+Started
...
plugin for Eclipse).  Make sure to check "Create a simple project (skip
archetype selection; we want to keep this simple).  When prompted, supply a
groupId and an artifactId.
...
https://blogs.oracle.com/coolstuff/entry/using_java_classes_in_jruby
...
"sandwich_spec.rb":https://github.com/dcvezzani/potential-dangerzone/blob/master/src/test/rspec/sandwich_spec.rb
for an example.
